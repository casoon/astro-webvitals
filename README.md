...
Open `/__web-vitals`. It shows the latest metric per route from this browser and can clear that local history. **Run sitemap pass** opens every page listed in `/sitemap.xml` in the current tab, waits briefly for each page to settle, and returns to the dashboard with local QA measurements. Alongside the metrics it shows browser-visible page checks for title, description, canonical, language, H1, Open Graph, skip link, alt attributes, link text, form labels and valid JSON-LD. It deliberately bypasses normal sampling for that explicitly started pass, while consent and Do Not Track still apply. This is a local QA complement to a build-time audit, not representative field data. Use `webVitalsDashboard({ route: '/internal/web-vitals', enabled: process.env.NODE_ENV !== 'production' })` to change or disable the injected route.
